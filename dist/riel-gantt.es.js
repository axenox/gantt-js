...
class Gantt {
  ...
  extend_gantt_range_to_include_today() {
    ...
    const today_scroll_view_end = this.get_today_scroll_padding_view_end_date(today_start);
    if (today_scroll_start < this.gantt_start) {
      this.gantt_start = date_utils.start_of(today_scroll_start, this.config.unit);
    }
    const required_today_end = today_scroll_view_end > today_end ? today_scroll_view_end : today_end;
  ...
  get_visible_viewport_units() {
    const viewport_width = this.$container?.clientWidth || this.$container?.parentElement?.clientWidth || this.$svg?.parentElement?.clientWidth || 0;
...
//# sourceMappingURL=riel-gantt.es.js.map
